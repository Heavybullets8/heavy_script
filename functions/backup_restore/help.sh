...
backup_help() {
    ...
    echo -e "    Restore all backups. If a backup name is provided, only restore that backup."
    echo -e "${blue}-S${reset}, ${blue}--restore-single${reset} [backup_name]"
    echo -e "    Restore a single backup. If a backup name is provided, only restore that backup."
