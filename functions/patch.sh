...
patch_2212_backups(){
    ...
    if ! patch -R --dry-run -N -s -p0 -d /usr/lib/python3/dist-packages/middlewared/ < patches/restore.patch &>/dev/null; then
        # If patch has not been applied, apply it
        if patch -N --reject-file=/dev/null -s -p0 -d /usr/lib/python3/dist-packages/middlewared/ < patches/restore.patch &>/dev/null; then
            echo "Restore Patch applied"
        else
        ...
        echo "Restore Patch already applied"
        exit 0
    fi
